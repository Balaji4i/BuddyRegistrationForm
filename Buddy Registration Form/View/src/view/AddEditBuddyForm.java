...
public class AddEditBuddyForm {
    ...
    public String onClickOfUnsubscribeButton() {
        ...
        if (!result) {
            ...
            xmlData =
                ApprovalPayload.businessTypeXMLData1(p_EmployeeName, p_EmployeeNumber, p_Email, p_BusinessGroup,
                                                     p_Cadre, p_AssignmentType, p_BuddyCareNo, p_TransDate,
                                                     p_DepartmentName, p_PreviousApprover, p_NextApprover,
                                                     p_ApprovalStatus, p_Comments, p_ApproverComments, p_KbcPeriodId,
                                                     p_KnowBuddyCareId, p_EffectiveStartDate, p_EffectiveEndate,
                                                     p_CurrentAvailablityInd);
            System.err.println("xmlData =>" + xmlData);
    ...
    public void onChangeofEffStartDate(ValueChangeEvent valueChangeEvent) {
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
        ViewObject periodVO = ADFUtils.findIterator("XssshrKbcPeriodVOIterator").getViewObject();
        Row currentRow = periodVO.getCurrentRow();
        RowSetIterator rs = periodVO.createRowSetIterator(null);
        String effstartDate = "";
        SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
        DateFormat formatterr = new SimpleDateFormat("dd-MM-yyyy");
        if (currentRow.getAttribute("EffectiveStartDate") != null) {
            Date currentEffStartDate = (Date) currentRow.getAttribute("EffectiveStartDate");
            effstartDate = formatterr.format(currentEffStartDate);
        }
        ...
        while (rs.hasNext()) {
            ...
            try {
                ...
                System.err.println("currEffStartDate--------" + currEffStartDate);
                System.err.println("effectiveStartDate--------" + effectiveStartDate);
                System.err.println("effectiveEndDate--------" + effectiveEndDate);

                if ((currEffStartDate).compareTo(effectiveStartDate) > 0 &&
                    ...
                    currentRow.setAttribute("EffectiveStartDate", "");
                    currentRow.setAttribute("EffectiveEndDate", "");
                    currentRow.setAttribute("EffectiveStartDate", null);
                    currentRow.setAttribute("EffectiveEndDate", null);
                }
    ...
    public void onChangeOfEffEndDate(ValueChangeEvent valueChangeEvent) {
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
        ViewObject periodVO = ADFUtils.findIterator("XssshrKbcPeriodVOIterator").getViewObject();
        Row currentRow = periodVO.getCurrentRow();
        if (currentRow != null && currentRow.getAttribute("EffectiveStartDate") == null) {
